...
문자열 : CHAR(고정문자열), VARCHAR(가변문자열)
    CHART 타입의 칼럼에 순수 숫자형문자열을 저장하면
    DV마다 특성이 달라 SELECT 등을 수행할때 불편함이 많다
    CHAR 타입을 사용하려면 반드시 아라펪 문자가 있는 형태로
    설계를 하는 것이 좋다.
    
    그래서 문자열의 경우 VARCHART를 사용해도 큰 문제가 없으므로
    고정 문자열로 VARCHAR로 사용하자
    
    CHAR : 최대 영숫자 2000 자까지
    VARCHAR : 최대 영숫자 4000 자 까지
    
    오라클 최근 버전에서는 VARCHAR, nVARCHART2, nVARCHAR 를 사용한다.
    
유니코드 문자열 : nCVAR, nVARCHAR
    영 숫자 이외의 2byte 형 문자(한글, 한자, 아랍어 등)울 저장하는 칼럼
...
일반적으로 Master Table 에는 코드체계를 만들어 PK를 설정한다
    학생테이블 = 학번이 PK, 사원테이블 = 사번이 PK
    과목테이블 = 과목코드 PK

전통적으로는 PK 는 정수형으로 설정하고, 일련번호를 부여하는 방식을 많이 사용한다.
    대부분의 DB 에는 AUTO INCREMENT 속성이 있어서 PK 칼럼을 설정한다.
    하지만 오라클 11 이하의 버전에는 AUTO UNCREMENT 속성이 아예 없다
...
현재 DBMS 프로그래밍에서는 Work Table 의 PK를 일련번호가 아닌
UUID 문자열로 하도록 권장한다.

*/

CREATE SEQUENCE seq_sample
INCREMENT BY 1  -- 시작값1 , 1씩증가
START WITH 1;
...
-- sample
SELECT 30*40 FROM tbl_sample; -- 테이블 개수만큼 결과를 보여라