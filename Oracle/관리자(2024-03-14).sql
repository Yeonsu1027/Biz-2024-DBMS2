...
특별하게 명령을 수행하면 이전 방법으로 사용자 생성을 할 수 있도록 했다

*/
-- 오라클 12c 이전의 표준 SQL 명령으로 사용자 생성을 할 수 있도록 하는 명령
-- 사용자생성을 하기 전에 항상 실행해야 한다
...
심지어 로그인을 할 수 잇는 권한도 없는 상태이다
사용자에게 부여하는 권한은 표준화(조직내에서)된 정책을 수립하고
...
CREATE TABLE user1.tbl_user (
    ...
    nickname nVARCHAR2(125) -- VARCHAR 은문자 nVARCHAR 한글포함알파벳 이외의문자
)
...
-- 오라클 DB  학습에서는 DBA라는 권한을 새로 생성한 사용자에게 부여한다
