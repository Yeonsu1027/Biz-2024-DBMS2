...
SELECT *
FROM tbl_iolist LIMIT 5 OFFSET 0;
...
-- 거래구분(io_div)이 2 이고
-- 거래수량(io_quan)이 50개 이하인 데이터만 SELECT 하시오.
SELECT *
FROM tbl_iolist
WHERE io_div = "2"
AND io_quan <= 50; 
...
SELECT io_pname AS 상품명,
    ...
    COUNT(io_pname) AS 횟수
FROM tbl_iolist
GROUP BY io_pname, io_div -- 상품이름으로 묶고, 다시 거래방법으로 나누어라 (1,2) 그다음에 개수를세라
ORDER BY io_pname; 