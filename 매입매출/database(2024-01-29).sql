...
삽입(추가)이상
    ...
    일부 항목이 누락되는 현상
    일부 항목의 값이 잘못 입력되는 형상
수정이상
	데이터를 변경(수정)할때 일부 데이터가
    수정되지 않거나, 또는 수정되어서는 안되는 데이터가
    변경되는 현상
삭제 이상
	데이터를 삭제할때 일부 데이터가 삭제 되지 않거나
    삭제 되어서는 안되는 데이터가 삭제되는 현상
...
WHERE io_pcode = p_code;     -- 두개의 테이블 동시에..  // 일치하는데이터를 한테이블로 출력

-- tbl_iolist 와 tbl_products 테이블을 join
-- Projection 하기              //분리된테이블을 조인해서 보고자하는 테이블로만든다
...
SELECT COUNT(*)
FROM tbl_iolist, tbl_products     -- iolist의 코드를 나열하고 pcode 와 일치하는데이터가 있으면 상품데이터에서 셀렉트
WHERE io_pcode = p_code; -- 상품정보 p_code는 primary키다.

SELECT * FROM tbl_products;
-- 상품 테이블에 pcode 가 P00014 인 상품을 삭제하기
DELETE FROM tbl_products
WHERE p_code ='P00014';       -- 50개에서 49개됐음

SELECT COUNT(*) FROM tbl_products;

SELECT COUNT(*)
FROM tbl_iolist, tbl_products -- 490 출력됨. 실제데이터에비해 보고있는것이 적다.
WHERE io_pcode = p_code;  -- 참조무결성이 무너졌음
...
null 로 출력하라
tbl_iolist 와 tbl_products 간에 "참조관계" 가 완전한지를
검증하는 코드로 사용된다.
*/        
        
SELECT io_pcode, p_name           -- 참조무결성을 검증하는 코드
FROM tbl_iolist        
	LEFT JOIN tbl_products  --  LEFT (outtter)JOIN  왼쪽에있는 테이블은 무조건 다 출력하라.
		ON io_pcode = p_code -- 그래서 io date는 500개 다나옴
ORDER BY p_name;
