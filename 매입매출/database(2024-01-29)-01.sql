...
CREATE TABLE tbl_iolist(io_seq	BIGINT		PRIMARY KEY,
...
FROM tbl_iolist   -- 이걸가지고
	LEFT JOIN tbl_products  -- 얠 셀렉트해
		ON io_pcode = p_code
WHERE p_name IS NULL; -- null이 있냐? 이걸실행했을때 데이터가 나오면 안됨.
...
ALTER TABLE tbl_iolist -- 테이블을 변경한다
ADD CONSTRAINT FK_PCODE
FOREIGN KEY (io_pcode)							-- 참조무결성이 아닐경우 오류
REFERENCES tbl_products(p_code);           -- 두테이블을 참조관계로 만들고 p code들을 연결해서 이름을 FK_PCODE 하겠다

SELECT * FROM tbl_iolist LIMIT 10;
-- tbl_iolist 에 ) P00012 상품의 거래데이터가 있기 때문에
-- tbl_products(상품테이블)에서 P00012 인 상품의 데이터는
-- 절대 삭제할 수 없게 되었다.
DELETE FROM tbl_products WHERE p_pcode = 'P00012';   -- 안됨. FK 는 삭제나 변경이 안된다.
