...
SELECT ST.st_num, ST.st_name,
    ...
    GROUP BY ST.st_num, ST.st_name;
				-- 칼럼의 결과물이같으면 ST.st_num 만써도 됨, 표준sql문은 SELECT에 2개있는거전부
...
SELECT ST.st_num, ST.st_name,
	...
	ROUND(AVG(SC.s_score),1) AS 평균 -- 소수점이하 1째자리까지
	FROM tbl_student ST
		JOIN tbl_score SC
			ON ST.st_num = SC.s_stnum
     WHERE ST.st_num = 'S0001'
...
SELECT ST.st_num, ST.st_name,
	MAX(IF(SC.s_subject = '국어', SC.s_score, 0)) AS 국어, -- 어차피 나머지 0이라 MAX도 가능한 거
...
-- WHERE 절에 PK 를 기준으로 조건주면 결과는 0..1 한 개의 객체    
